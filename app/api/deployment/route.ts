...
export async function GET() {
  // Local development runs Next.js on the host and the agent on port 8000.
  // Docker Compose overrides this with http://agent:8000, keeping the two
  // stacks isolated instead of leaking a Docker-only hostname into local dev.
  const baseUrl = process.env.AGENT_INTERNAL_URL || 'http://127.0.0.1:8000'
