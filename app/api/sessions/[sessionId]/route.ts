...
async function forwardLifecycle(method: 'DELETE' | 'POST', context: RouteContext) {
  ...
  if (!token) {
    return NextResponse.json(
      { error: 'Server session deletion is not configured' },
      { status: 503 },
    )
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Agent cleanup failed' },
      { status: 502 },
    )
  }
}
...
function localDevelopmentToken(): string {
  // Next loads env files from oo-chat/, while the local Python agent intentionally
  // owns the repository-root .env. Read only the lifecycle/model token names so
  // local development uses the same authenticated endpoint as Docker deployment.
  try {
